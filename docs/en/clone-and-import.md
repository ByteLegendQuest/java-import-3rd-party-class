...
`git clone https://github.com/ByteLegendQuest/java-import-3rd-party-class`

A project directory named `java-import-3rd-party-class` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-import-3rd-party-class` directory which is created just now.
